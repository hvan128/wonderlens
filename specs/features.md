...
- [ ] Hoạt động offline (audio asset hoặc on-device TTS)
- [ ] Tiếng Việt, giọng phù hợp trẻ em

**Note:** Dùng audio asset pre-gen cho hero cố định khi có (`Stage.audio`, ví dụ `paper_cup`), fallback `flutter_tts` để vẫn offline nếu asset thiếu.
...
- [x] Hoàn tất hoặc bỏ qua onboarding seed sẵn `paper_cup` vào bộ sưu tập để home không rỗng sau lần đầu
...
## F-10: WonderLens Plus subscription foundation

**Priority:** P2 | **Status:** Done | **Domain:** Subscription & Parent Controls | **Ref:** [ADR-014](../adrs/ADR-014-subscription-paywall.md), [TASK-014](../tasks/TASK-014-subscription-paywall.md)

**Description:**
Màn paywall WonderLens Plus cho phụ huynh, kết nối StoreKit / Google Play Billing
qua `in_app_purchase`, kèm fallback mock local khi Store chưa trả về product IDs.

**User story:**
*Phụ huynh mở Hồ sơ, xem gói WonderLens Plus, qua parental gate rồi mua/restore
subscription bằng Store sheet khi sản phẩm đã được cấu hình.*

**Acceptance criteria:**
- [x] Hồ sơ có lối vào WonderLens Plus và trạng thái Plus.
- [x] Paywall minh bạch, tiếng Việt, theo visual WonderLens.
- [x] Có lựa chọn gói năm trial 3 ngày và gói tháng.
- [x] CTA đi qua parental gate.
- [x] Product IDs cấu hình qua dart-define và query từ Store.
- [x] Purchase/restore cập nhật entitlement từ purchase stream.
- [x] Trạng thái Plus persist local qua Hive.
- [x] Restore Purchase phản hồi rõ.
- [x] Privacy/Terms truy cập được từ paywall.

**Note:** Giao dịch thật chỉ chạy khi Store trả product IDs khớp dart-define.
`app/scripts/create_store_subscriptions.rb` tạo metadata qua API. iOS đã có
product IDs/giá/trial/review screenshot; Google Play cần bật payments profile
trước khi API cho tạo subscription. Nếu Store chưa trả sản phẩm, app dùng
fallback mock nội bộ và hiển thị rõ là chưa thu tiền thật.

---

## F-11: Mission onboarding + local reminders

**Priority:** P1 | **Status:** Done | **Domain:** Cross-cutting | **Ref:** [ADR-015](../adrs/ADR-015-local-mission-reminders.md), [TASK-015](../tasks/TASK-015-mission-reminders.md)

**Description:**
Phụ huynh có thể bật "Nhắc khám phá" trong Hồ sơ. App đặt một local notification
sau 2 ngày không mở app; tap notification mở thẳng onboarding capsule của một
hero object curated/offline.

**User story:**
*Phụ huynh thấy lời nhắc "Hôm nay soi thử: Bút bi" → chạm notification → app mở
màn chụp thử bút bi → reveal → timeline thật để cùng bé xem bút bi được tạo ra
như thế nào.*

**Acceptance criteria:**
- [x] Có route `/onboarding/mission/:objectId`.
- [x] Notification payload `mission:<object_id>` mở đúng mission.
- [x] Reminder mặc định tắt, bật/tắt trong Hồ sơ.
- [x] Notification copy parent-facing, không FOMO/streak, không spam.
- [x] Chỉ dùng hero object curated/offline; id không hợp lệ fallback `paper_cup`.
- [x] Ảnh mission được pre-gen bằng OpenAI Images qua script repo, không nhúng key
      vào app.
- [x] Mission/onboarding dùng cutout của object làm visual; không dùng emoji thay
      ảnh vật.
- [x] App reschedule reminder khi mở lại/resume.
- [x] Nhấn giữ card "Nhắc khám phá" schedule notification test sau 10 giây.

**Non-goals:**
- Không remote push/Firebase token trong pha này.
- Không gửi AI-live object qua notification.
- Không dùng notification cho quảng cáo hoặc subscription.
